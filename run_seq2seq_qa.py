...
def main():
    ...
    def post_processing_function(
        ...
        for i in range(len(decoded_preds)):
            predictions[examples["id"][i]] = decoded_preds[i]
        '''
    ...
    if training_args.do_predict:
        ...
        with open(training_args.output_dir + "/predictions.json", "w") as f:
            json.dump(results.predictions, f, indent = 2)
        with open(training_args.output_dir + "/labels.json", "w") as f:
            json.dump(results.label_ids, f, indent = 2)
        print(results.predictions, type(results.predictions))
        # for i, j in (results.label_ids, results.predictions):
